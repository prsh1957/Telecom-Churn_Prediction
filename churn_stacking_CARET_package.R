...
##Group factors many levels into factors with 3-4 levels
...
feature_Engg_param<-function(df_orig, df_imputed){
  ...
  ohefit=dummyVars(churn~.,data = data2[,-58])
  data2_d=as.data.frame(predict(ohefit, data2)) #this returns a matrix which needs to be converted
...
##Feature Engineering for non-parametric methods like Tree
...
feature_Engg_nonparam<-function(df_orig, df_imputed){
  ...
  data2=select(data2,-'Customer_ID')
...
modelfit<-function(data,data_imputed,train_params, specific_params, 
  ...
  data_train$churn=ifelse(data_train$churn== 0,'No','Yes')
  
  data_train$churn=as.factor(data_train$churn)
  
  data_test$churn=ifelse(data_test$churn== 0,'No','Yes')
...
CARET.TUNE.GRID <-  expand.grid(nrounds=c(350,400,450), 
                                max_depth=c(4,5), 
                                eta=c(0.05,0.075,0.1), 
                                gamma= 0.6, 
                                colsample_bytree=c(0.9,0.95), 
...
CARET.TRAIN.CTRL <- trainControl(method="repeatedcv",number = 5,
                                 repeats=1, verboseIter=FALSE,
                                 classProbs=TRUE,
...
CARET.TUNE.GRID <-  expand.grid(n.trees=c(300,350,400), 
                                interaction.depth=c(4,5,6), 
                                shrinkage=0.1,
                                n.minobsinnode=c(200,250,300))
...
CARET.TUNE.GRID <-  expand.grid(mtry=c(5,10,15,20,25))
...
names(level1_train)=c('churn','xgb_probs', 'gbm_probs', 'rf_probs','lr_probs')
...
names(level1_test)=c('churn','xgb_probs','gbm_probs','rf_probs','lr_probs')
...
CARET.TUNE.GRID <- expand.grid(size=c(2,3,4,5,6),decay=c(0.01,0.001))    # NULL provides model specific default tuning parameters
